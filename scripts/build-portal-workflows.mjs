...
function workflow(name,path,method,code,respondWith='text') { return {name,active:false,settings:{executionOrder:'v1',availableInMCP:false},nodes:[{parameters:{httpMethod:method,path,responseMode:'responseNode',options:{}},id:'trigger',name:'Webhook',type:'n8n-nodes-base.webhook',typeVersion:2.1,position:[-280,0],webhookId:`postagem-redes-${path}`},{parameters:{jsCode:code},id:'code',name:'Processar',type:'n8n-nodes-base.code',typeVersion:2,position:[0,0]},{parameters:respondWith==='binary'?{respondWith:'binary',responseBinaryPropertyName:'data',options:{responseHeaders:{entries:[{name:'Cache-Control',value:'private, max-age=604800, immutable'}]}}}:{respondWith:'text',responseBody:'={{ $json.html }}',options:{responseHeaders:{entries:[{name:'Content-Type',value:'text/html; charset=utf-8'},{name:'Cache-Control',value:'no-store'}]}}},id:'respond',name:'Responder',type:'n8n-nodes-base.respondToWebhook',typeVersion:1.5,position:[260,0]}],connections:{Webhook:{main:[[{node:'Processar',type:'main',index:0}]]},Processar:{main:[[{node:'Responder',type:'main',index:0}]]}},pinData:{}}; }
function actionWorkflow() {
 ...
 const failure=node('delivery-failure','Registrar falha e retry','n8n-nodes-base.code',2,{jsCode:deliveryFailureCode},[2580,960]);
 const success=node('delivery-success','Registrar publicação e histórico','n8n-nodes-base.code',2,{jsCode:deliverySuccessCode},[2760,720]);
 ...
 return {
  ...
  nodes:[
   ...
   node('portal-actions-gemini','Gemini · fallback','@n8n/n8n-nodes-langchain.googleGemini',1.2,{resource:'text',operation:'message',modelId:{mode:'id',value:'={{ $vars.SOCIAL_GEMINI_MODEL || "gemini-2.5-flash" }}'},messages:{values:[{role:'user',content:'={{ $("Processar ação do portal").item.json.aiPrompt }}'}]},simplify:true,jsonOutput:true,options:{systemMessage:'Retorne somente JSON válido conforme o pedido; não use markdown.',maxOutputTokens:1800,temperature:0.3}},[640,-160],{onError:'continueErrorOutput'}),
   ...
   failure, success,
   node('ledger-insert','Registrar no Data Table · Ledger','n8n-nodes-base.dataTable',1.1,{resource:'row',operation:'insert',dataTableId:{mode:'name',value:'Postagem Redes - Ledger'},columns:{mappingMode:'defineBelow',value:{content_id:'={{ $json.contentId }}',network:'={{ $json.network }}',status:'={{ $json.ok ? "published" : "failed" }}',remote_id:'={{ $json.remoteId || "" }}',permalink:'={{ $json.permalink || "" }}',error:'={{ $json.error || "" }}',dispatch_id:'={{ $json.dispatchId }}',occurred_at:'={{ $now.toISO() }}'}},options:{optimizeBulk:true}},[3000,820],{onError:'continueErrorOutput'}),
  ],
  connections:{
